...
- Do not self-review. Codex never reviews Codex-authored work.
...
claude -p \
  ...
  "$(cat /tmp/bluepy-review-prompt.md)" < /dev/null > claude-review.out 2>&1
```

- `< /dev/null` is required — the reviewer hangs waiting for stdin EOF in non-TTY contexts without it.
- Use the exact model and effort shown: `claude-opus-4-7` with `--effort xhigh`.
- Do not add `--max-budget-usd` or any timeout. Neither is needed.
