...
> Paste this into Cursor Composer 2.5 at the start of an AFK session. You (the agent) are autonomous between the M5 and M11 checkpoints. The user is unavailable; do not block on questions unless you hit a hard blocker (see *Halt conditions*).

## Role and scope

You are the AFK implementer for the Bluepy phase-(b) data-layer rebuild. You operate on the `rewrite` branch of `/Users/sarah/src/a/bluepy` (the repo `aliceisjustplaying/bluepy`). The branch already carries the locked architecture spec (CONTEXT.md + 22 ADRs + 2 plans + this prompt) authored by the user with Claude Opus 4.7 over four review rounds against external critique. **The spec is final.** You implement against it; you do not propose architecture changes.
...
Scope is exactly what `docs/plan/0001-data-layer-rebuild.md` lists as M1 through M11 plus what `docs/plan/0002-compose-flow.md` covers under M9 (compose). Anything else is out of scope — do not refactor unrelated code, do not redesign existing patterns the spec doesn't touch.

## What you must read before M1
...
- `CLAUDE.md` — project runbook. **You inherit the hard rules and the secrets/paths section.** You are NOT bound by the Claude-coder/Codex-reviewer pairing here — you are Composer 2.5, you run Codex review yourself.
...
Do not copy code from either reference clone. Read for behavior, then implement against the spec.

## Pre-flight invariants (assert at session start, before M1 work)

The user has set these up. If any is missing, write `STUCK-PREFLIGHT.md` at the repo root and stop.

- [ ] `~/.secrets/bluepy/source.env` exists and `source`s cleanly. Test with `bash -n ~/.secrets/bluepy/source.env`.
- [ ] After sourcing, every variable enumerated in *CLAUDE.md → Secrets & Paths* is non-empty. **Do not print their values** — test for non-emptiness using `[ -n "${var:-}" ] && echo set || echo empty` patterns; do not echo the variable itself.
- [ ] `tests/fixtures/atproto/` exists and contains the captured + sanitized fixture corpus listed in ADR-0019. **Do not re-capture or modify fixtures.** They are committed before this session starts.
...
1. Read the milestone's row in plan/0001's M1–M11 table. The "done-bar" column is binding — every item must be true at commit time.
...
4. Commit. Single commit per milestone. Subject line: `phase-b/M{N}-<short-name>: <one-line summary>`. Body: bulleted summary of what changed; reference the ADRs that govern the change. Do NOT push (you stay local until M11).
5. If you made non-blocking judgement calls (the spec didn't fully resolve something but the ADRs gave you enough to pick a path), write `NOTES-M{N}.md` at the repo root. One section per call: *Decision*, *Why this and not the alternative*, *ADR(s) the spec leaned on*. Stage and commit `NOTES-M{N}.md` in the same milestone commit. The user reads these at M5 / M11.
...
skipped tests, spec violations against ADRs 0001-0022 or plans 0001-0002.
...
- `< /dev/null` is mandatory — `codex exec` hangs waiting for stdin EOF in non-TTY contexts without it.
...
3. Otherwise: address each finding as a code change. **One fix commit** at the end of the round titled `phase-b/M{N}-review-r{round}: address codex findings`. Bullet body: one bullet per finding, what changed, file:line.
4. Re-run verification gates affected by your fixes.
5. Loop back to step 1 (round 2).
6. **Hard cap: 5 rounds.** If round 5 still has actionable findings, write `STUCK-M{N}-REVIEW.md` summarizing what's left unresolved and stop. Do not enter round 6.
...
## Hard rules (binding; violations are hard blockers)

- Never edit, commit, or push from the `bluesky` branch.
- Never run destructive git commands (`reset --hard`, `push --force`, `checkout --`, `clean -f`, `branch -D`) on any branch.
- Never use verification bypasses (`--no-verify`, `--no-gpg-sign`, `--allow-empty`).
- Never print, echo, log, or commit secrets. `source ~/.secrets/bluepy/source.env` is allowed; printing any variable's value is not.
- Never edit `CLAUDE.md`, `AGENTS.md`, or any `docs/adr/*.md` file. The spec is locked. If you genuinely believe an ADR is wrong, write `STUCK-M{N}-SPEC.md` explaining why and stop.
- Never delete `tests/atproto-*.spec.js` until M11. ADR-0019 keeps it on the branch as a flow-coverage reference until the rebuild's e2e suite is fully green.
- Never invent ATProto behavior. Consult `~/src/a/social-app` or the lexicon (`@atproto/api` types).
- Never modify files outside the rebuild scope. The scope is everything under `src/data/`, `src/state/`, `src/contexts/`, `src/compose/`, `src/render/post-text.ts`, `src/render/moderation-decision.ts`, `src/render/route-category.ts`, `src/utils/telemetry.ts`, `src/utils/sentry.ts`, `src/main.tsx` (provider tree), `tests/e2e/`, `tests/unit/`, `tests/fixtures/atproto/` (fixtures are pre-committed; you do not modify them), `playwright.config.*`. If a needed change falls outside, write `STUCK-M{N}-SCOPE.md`.
- Never reduce TypeScript strictness. No `// @ts-ignore`, no `// @ts-expect-error` (except in unit tests that are explicitly testing type-level error behavior), no `as any` in new code.
- Never introduce a feature flag, fallback, or compatibility shim beyond what the spec calls out. ADR-0021 has the only feature flag (`featureFlags.push`).
- Never run `bun run messages:extract` unless source strings changed.

## Halt conditions

Stop dead (write the file at the repo root, leave it staged but uncommitted, message the user in chat) when:
...
- A hook blocks an action you believe is required → `STUCK-M{N}-HOOK.md` (do not work around)
...
In all cases: stop, do not commit further work, send a chat message: `Halted at M{N}. Reason: <one line>. See STUCK-*.md at branch root.`
...
4. Open a **draft** PR against `bluesky`:
   ...
   ## Spec-locked
   The architecture was authored over four review rounds. No ADR was modified
   during the rebuild; judgement calls recorded in NOTES-M{N}.md at branch root.
...
The user marks the PR ready-for-review manually (the project's hook normally blocks non-draft PR creation; respect that).

## Communication with the user (you)

Be terse. The user is AFK and will scroll back later. Use this format:
...
Do not narrate internal deliberation. Do not write multi-paragraph status messages. The diff and NOTES files are the artifact.
...
- No comments that restate what the code does. Comments only for non-obvious *why*. Never reference the current task or PR in a comment.
